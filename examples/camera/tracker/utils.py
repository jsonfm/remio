...
class Tracker(object):
    ...
    def get_particle_position(self, image: np.ndarray, mask: np.ndarray, origin: list, 
        draw_contour: bool = True, color_contour: np.ndarray = None, translate: bool = True):
        ...
        if draw_contour:
            cv2.drawContours(image, contour, -1, color_contour, 2)  # draw the contour of my object
    ...
    @staticmethod
    def draw_grid(image: np.ndarray, grid_size: list = [10, 10], grid_color: tuple = (0, 0, 0)):
    ...
    def draw_text_point(self, image: np.ndarray = None, point: tuple = (20, 20), text: str = "A", 
        text_color: tuple = (255, 255, 255), point_color = (255, 255, 255), radius=10,
        offset_x: int = 10, offset_y: int = 10, font = cv2.FONT_HERSHEY_SIMPLEX,
        font_scale: float = 0.7, thickness=2):
        ...
        cv2.putText(image, text, (xt, yt),font, font_scale, text_color, thickness,
                    cv2.LINE_AA)
        return image
    ...
    def track(self, image: np.ndarray, *args, **kwargs):
        ...
        image = self.draw_text_point(image, point=position_translated, text=f"P{position}")        
        cv2.arrowedLine(image, origin, position_translated, (255, 255, 255), 2, line_type=8, tipLength=0.3)
